...
import { fetchTaskById, submitTaskProof } from '@/services/microtasks.service';
import { formatINR } from '@/services/wallet.service';
import type { Microtask } from '@/types';

export default function MicrotaskDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleSubmitProof = async (e: React.FormEvent) => {
    ...
    // Validate required proofs based on task.proofTypes
    if (task.proofTypes.includes('text') && !proofText.trim()) {
      setError('Please provide the required Text proof / note.');
      return;
    }
    if (task.proofTypes.includes('url') && !proofUrl.trim()) {
      setError('Please provide the required verification URL link.');
      return;
    }
    if (task.proofTypes.includes('screenshot') && proofScreenshots.length === 0) {
    ...
    return (
      <DashboardLayout>
        <div className="py-12 text-center space-y-4">
          ...
          <h2 className="text-xl font-bold">Microtask Not Found</h2>
          ...
          <div className="bg-slate-950 p-6 md:p-8 text-white relative">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <h1 className="font-display text-2xl md:text-3xl font-bold tracking-tight">{task.title}</h1>
                <p className="text-slate-300 text-sm max-w-2xl">{task.description}</p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="card-premium p-6 flex flex-col justify-between">
            <form onSubmit={handleSubmitProof} className="space-y-5">
              ...
              {/* Screenshot Proof */}
              {task.proofTypes.includes('screenshot') && (
